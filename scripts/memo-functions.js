...
window.addEventListener('load', function() {
    ...
    // Saves memo on form submit.
    memoForm.onsubmit = function(e) {
        e.preventDefault();
        var title = titleInput.value;
        var content = contentInput.value;
        if (title && content) {
            newMemoForCurrentUser(title, content).then(function() {
                myMemosMenuButton.click();
            });
            contentInput.value = '';
            titleInput.value = '';
            memoIsActiveInput.value = false;
            memoDifficultyInput.value = '';
            memoTopicsInput.value = [];
            memoRangeInput.value = 50;
            fileInput.value = '';
            linkbox.value = '';
            linkbox.innerHTML = '';
        }
    ...
    */
    ...
    //uploadFile();
    memoIsActiveInput.addEventListener('change', function() {
        isActive = $(this).is(":checked");
        console.log(isActive);
    });

    memoDifficultyInput.addEventListener('change', function() {
        difficulty = this.value;
        console.log(difficulty);
    });

    memoTopicsInput.addEventListener('change', function() {
        topicsSelected = [];
        ...
        memoTopicsHelp.innerHTML = topicsSelected;
